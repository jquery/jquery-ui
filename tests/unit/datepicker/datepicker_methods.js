...
test("destroy", function() {
	...
	ok($.data(inp[0], datepickerTestHelper.PROP_NAME), "Default - instance present");
	ok(inp.next().is("#alt"), "Default - button absent");
	inp.datepicker("destroy");
	inp = $("#inp");
	ok(!inp.is(".hasDatepicker"), "Default - marker class cleared");
	ok(!$.data(inp[0], datepickerTestHelper.PROP_NAME), "Default - instance absent");
	ok(inp.next().is("#alt"), "Default - button absent");
	// With button
	inp= datepickerTestHelper.init("#inp", {showOn: "both"});
	ok(inp.is(".hasDatepicker"), "Button - marker class set");
	ok($.data(inp[0], datepickerTestHelper.PROP_NAME), "Button - instance present");
	ok(inp.next().text() === "...", "Button - button added");
	inp.datepicker("destroy");
	inp = $("#inp");
	ok(!inp.is(".hasDatepicker"), "Button - marker class cleared");
	ok(!$.data(inp[0], datepickerTestHelper.PROP_NAME), "Button - instance absent");
	ok(inp.next().is("#alt"), "Button - button removed");
	// With append text
	inp = datepickerTestHelper.init("#inp", {appendText: "Testing"});
	ok(inp.is(".hasDatepicker"), "Append - marker class set");
	ok($.data(inp[0], datepickerTestHelper.PROP_NAME), "Append - instance present");
	ok(inp.next().text() === "Testing", "Append - append text added");
	inp.datepicker("destroy");
	inp = $("#inp");
	ok(!inp.is(".hasDatepicker"), "Append - marker class cleared");
	ok(!$.data(inp[0], datepickerTestHelper.PROP_NAME), "Append - instance absent");
	ok(inp.next().is("#alt"), "Append - append text removed");
	// With both
	inp= datepickerTestHelper.init("#inp", {showOn: "both", buttonImageOnly: true,
		buttonImage: "images/calendar.gif", appendText: "Testing"});
	ok(inp.is(".hasDatepicker"), "Both - marker class set");
	ok($.data(inp[0], datepickerTestHelper.PROP_NAME), "Both - instance present");
	...
	ok(!$.data(inp[0], datepickerTestHelper.PROP_NAME), "Both - instance absent");
	...
	ok($.data(inl[0], datepickerTestHelper.PROP_NAME), "Inline - instance present");
	...
	ok(!$.data(inl[0], datepickerTestHelper.PROP_NAME), "Inline - instance absent");
	ok(inl.next().length === 0 || inl.next().is("p"), "Inline - button absent");
});
...
test("enableDisable", function() {
	...
	inp.datepicker("destroy");
	// With a button
	inp = datepickerTestHelper.init("#inp", {showOn: "button"});
	...
	inp = datepickerTestHelper.init("#inp", {showOn: "button", buttonImageOnly: true,
	...
	inp.datepicker("destroy");
	// Inline
	inl = datepickerTestHelper.init("#inl", {changeYear: true});
