...
$.widget("ui.dialog", {
	...
	_create: function() {
		...
		this.options.indexInParent = $(this.element).parent().find('*').index($(this.element));
	...
	_destroy: function() {
		...
		if(parentHaveChildrens) {
			...
			if (iWasFirstChild) {
				neighbor = parent.find('*').eq(this.options.indexInParent);        
			}
			else {
				neighbor = parent.find('*').eq(this.options.indexInParent - 1);      
			}
		}
